...
public class Wolf extends Animal {
    ...
    protected void move() {
        synchronized (Game.lock){
            ...
            for(int i =0; i<Game.rabbits.size(); i++){
                if(!Game.rabbits.get(i).isDead()) {
                    if (Game.rabbits.get(i).getX() == x + direction.getX() && Game.rabbits.get(i).getY() == y + direction.getY()) {
                        collide = true;
                        rid = i;
                        break;
                    }
    ...
    protected Point randomDirection(){
        ...
        try {
            ...
            angle = (float) Math.toDegrees(Math.atan2(Game.rabbits.get(rid).getY() - y, Game.rabbits.get(rid).getX() - x));
